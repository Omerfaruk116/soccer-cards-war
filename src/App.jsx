...
if (
  ...
  !window[INSTALL_CAPTURE_KEY]
) {
  window[INSTALL_CAPTURE_KEY] = true;
...
const RENTAL_MAX_HOURS = 24;
const RENTAL_FULL_INCOME = 12000;
...
function comparePlayersStrongestFirst(
  a,
  b
) {
...
function sortPlayersStrongestFirst(
  players = []
) {
  return [...(players || [])]
    .filter(Boolean)
...
function App() {
  ...
  async function installGame() {
    ...
    if (prompt) {
      try {
        ...
        // Tarayıcı kendi kurulum penceresini yönetir.
      }
    }

    showToast(
      "📲 Tarayıcı menüsünden ‘Uygulamayı yükle’ seçeneğini aç. Chrome/Edge’de adres çubuğundaki yükleme simgesini de kullanabilirsin.",
      "info"
    );
  }
    ...
    setCoachPlayerIds(
      (current) => {
        ...
        if (
          ...
          showToast(
            `Bu antrenör en fazla ${coach.slots} oyuncuyla çalışabilir.`,
            "error"
          );
          return current;
        }
  ...
  if (
    ...
    const activeCoachSessions =
      game.coaches?.activeSessions || [];
    ...
    const selectedCoachBusy =
      selectedCoach
        ? activeCoachSessions.some(
            (session) =>
              session.coachId ===
                selectedCoach.id &&
              Number(
    ...
    content = (
      <section className="screen">
        ...
        <SectionHeader
          ...
          right={
            <StatChip>
              ⭐ {ownedCoachIds.length}/{COACHES.length}
            </StatChip>
          }
        />

        {activeCoachSessions.length > 0 && (
          <div
            style={{
              display: "grid",
              gap: 10,
            ...
            (coach) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {coach.slots} oyuncu • {coach.minutes} dk
                  </div>
        ...
        {selectedCoach &&
          ownedCoachIds.includes(
            ...
            >
              ...
              <h2>
                OYUNCULARI SEÇ • {coachPlayerIds.length}/{selectedCoach.slots}
              </h2>

              {selectedCoachBusy ? (
                <p className="muted">
                  Bu antrenörün aktif seansı bitince yeni çalışma başlatabilirsin.
                </p>
              ) : (
                <>
                  <p className="muted">
        ...
        <div className="card-grid">
                  ...
                  >
                    ⭐ SEANSI BAŞLAT • {selectedCoach.minutes} DK
                  </button>
                </>
              )}
            </div>
        ...
        {!game.rentalCenter
          ...
          <div className="panel">
            ...
            <p className="muted">
              Kullanmadığın
              oyuncuları maksimum
              24 saat kiraya ver ve
              🪙12.000 Coin kazan. Erken geri çağırırsan geçen süre kadar ödeme alırsın.
            </p>
    ...
    content = (
      <section className="screen">
        ...
        >
          ...
          <p className="muted">
            Soccer Cards War'ı cihazına uygulama gibi kur. Profil ekranından bu düğmeye her zaman ulaşabilirsin.
          </p>
